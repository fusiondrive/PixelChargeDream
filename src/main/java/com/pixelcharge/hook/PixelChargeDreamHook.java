...
public final class PixelChargeDreamHook implements IXposedHookLoadPackage {
    ...
    private void hookDreamlinerProcess(LoadPackageParam lpparam) {
        // 1. Force ALL SharedPreferences boolean queries in Dreamliner to true (Phenotype numeric IDs & named flags)
        try {
        ...
        // 2. Force ALL PhenotypeFlag getters to true
        try {
        ...
        // 3. Block setComponentEnabledSetting from disabling anything in Dreamliner
        try {
            ...
            Log.w(TAG, "Dreamliner ApplicationPackageManager hook skipped: " + t.getMessage());
        }
    }

    private void hookSettingsProcess(LoadPackageParam lpparam) {
        // Hook DreamPickerController in Settings
        try {
